...
public class Solution
{
    ...
    public static Map<String, Integer> countWords(ArrayList<String> list)
    {
        ...
        for (int i = 0; i < list.size(); i++) {
            counter = 1; isStart = true;
            temp = list.get(i).toLowerCase();
            for (int j = 0; j < list.size(); j++) {
                if (temp.equals(list.get(j).toLowerCase()))
                {
                    if (!result.containsKey(list.get(j)) && isStart)
                    {
                        result.put(list.get(j), counter);
                        isStart = false;
                    } else if (result.containsKey(list.get(j)) && !isStart)
                    {
                        counter++;
                        result.put(list.get(j), counter);
                    }
